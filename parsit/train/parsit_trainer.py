...
def get_modality_length_grouped_indices(lengths, batch_size, world_size, generator=None):
    ...
    # Mega-batches are themselves batches of batches grouped by modality
    image_indices = [i for i in sorted_indices if lengths[i][1] > 0]
    text_indices = [i for i in sorted_indices if lengths[i][1] == 0]

    # Mix the modalities by block
    text_batch_size = batch_size // 4 if len(image_indices) > 0 else batch_size
    image_batch_size = batch_size - text_batch_size

    text_megabatches = [text_indices[i : i + text_batch_size * world_size] for i in range(0, len(text_indices), text_batch_size * world_size)]
    image_megabatches = [image_indices[i : i + image_batch_size * world_size] for i in range(0, len(image_indices), image_batch_size * world_size)]

    megabatches = []
    for i in range(max(len(text_megabatches), len(image_megabatches))):
        batch = []
        if i < len(text_megabatches):
            batch.extend(text_megabatches[i])
        if i < len(image_megabatches):
            batch.extend(image_megabatches[i])
        megabatches.append(batch)

    # Randomly permute the batches
    shuffled_indices = []
    for megabatch in megabatches:
        megabatch = sorted(megabatch, key=lambda i: indices[i], reverse=True)
        shuffled_indices.extend(megabatch)
...
class LengthGroupedSampler(Sampler):
    ...
    def __init__(
        ...
        if lengths is None:
            raise ValueError("Lengths must be provided to the sampler.")
    ...
    def __iter__(self):
        if self.variable_length:
            ...
            indices = get_length_grouped_indices_hf(self.lengths, self.batch_size, generator=self.generator)
        return iter(indices)
